...
function spawnGrain(W, H, idx) {
  ...
  const cx = W / 2,
    cy = H / 2;
  const toAngle = Math.atan2(cy - y + rand(-H * 0.2, H * 0.2), cx - x + rand(-W * 0.2, W * 0.2));
  const spd = rand(6, 16);
  ...
  return {
    ...
    spawnX: x,
    spawnY: y,
    vx: Math.cos(toAngle) * spd,
    vy: Math.sin(toAngle) * spd,
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      if (ph === 'erupt') {
        ...
        const bg = ctx.createRadialGradient(cx * 0.6, cy * 0.5, 0, cx, cy, DIAG * 0.65);
        ...
        if (t < 0.75) {
          [
            ...
            const g = ctx.createRadialGradient(bx, by, 0, bx, by, DIAG * 0.52 * easeOut(t));
            g.addColorStop(0, `rgba(235,155,45,${0.82 * (1 - t * 0.5)})`);
            g.addColorStop(0.45, `rgba(185,102,18,${0.55 * (1 - t * 0.4)})`);
      ...
      if (ph === 'bang') {
        ...
        if (t < 0.85) {
          ...
          const glow = ctx.createRadialGradient(
            cx,
            cy,
            Math.max(0, ringR - ringW * 1.5),
            cx,
            cy,
            ringR + 4
          );
          glow.addColorStop(0, `rgba(${target.c1.join(',')},0)`);
          glow.addColorStop(0.5, `rgba(${target.c1.join(',')},${0.9 * (1 - t * 0.6)})`);
          glow.addColorStop(0.85, `rgba(${target.c2.join(',')},${0.7 * (1 - t * 0.5)})`);
        ...
        if (t < 0.6) {
          const offset = 18 * (1 - t);
          [
            [target.c1, 'rgba(0,0,0,0)'],
            [target.c3, 'rgba(0,0,0,0)'],
          ].forEach(([col], ci) => {
            ...
            ctx.beginPath();
            const cr2 = waveR + offset * 2;
            const cr1 = Math.max(0, cr2 - 22);
        ...
        for (let i = 0; i < rays; i++) {
          ...
          const len = lerp(0, DIAG * 0.75, easeOut(t));
          const rayA = (1 - t) * 0.22;
          ...
          rg.addColorStop(0.5, `rgba(${target.c2.join(',')},${rayA * 0.6})`);
          rg.addColorStop(1, 'rgba(0,0,0,0)');
          ctx.beginPath();
          ctx.moveTo(0, -2 + (i % 3));
          ctx.lineTo(len, -(1 + (i % 2)));
          ctx.strokeStyle = rg;
      ...
      if (ph === 'settle') {
        ...
        const resG = ctx.createRadialGradient(cx, cy, 0, cx, cy, DIAG * 0.45);

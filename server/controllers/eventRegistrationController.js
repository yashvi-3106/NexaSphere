...
export const cancelRegistration = wrapAsync(async (req, res) => {
  ...
  if (authenticatedEmail !== sanitizedEmail) {
    return sendError(req, res, 'Forbidden: you can only cancel your own registration', 403, 'FORBIDDEN');
  }
...
export const confirmWaitlistSpot = wrapAsync(async (req, res) => {
  ...
  if (!confirmed) {
    return sendError(req, res, 'No pending waitlist promotion found for this email', 404, 'NOT_FOUND');
  }

...
import { readContent, writeContent, DEFAULT_CONTENT, ensureContentFile } from '../storage/contentFileStore.js';
...
test('eventsService CRU and delete (file store)', async () => {
  ...
  const created = await eventsService.createEvent({ name: 'TS Test Event', date: '2026-05-16', description: 'desc' });
  ...
  const updated = await eventsService.updateEvent(created.id, { name: 'TS Updated', description: 'new' });
